...
if len(sys.argv) != 5:
    print("Usage: ./bin/spark-submit Mansi_Ganatra_task2.py <filter_threshold> <support> <input_file_path> <output_file_path>")
    exit(-1)
else:
...
def apriori_implementation(baskets, support_threshold, total_baskets_count):
    ...
    # print("Current partition threshold: " + str(partition_support_threshold))
...
# output_file_path = "./task2__result.json"
# supportThreshold = 70
# filter_threshold = 40
...
finalRdd = finalRdd.map(lambda entry: (entry[0], entry[1]))\
    .groupByKey()\
    .mapValues(lambda entry: set(entry)).filter(lambda entry: len(entry[1]) > filter_threshold).persist()
...
# print("Total Baskets: " + str(total_baskets_count))
